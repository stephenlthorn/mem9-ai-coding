...
// ── Cross-repo: one task, two repos ───────────────────────────────────────────
let _xrepoInit = false;
function initCrossRepo() {
  if (!_xrepoInit) {
    _xrepoInit = true;
    $('xrepo-run').addEventListener('click', async () => {
      const btn = $('xrepo-run');
      btn.textContent = 'Running…'; btn.disabled = true;
      await postJSON('/api/cross-repo/run');
      await new Promise(r => setTimeout(r, 1600));
      await loadCrossRepo();
      btn.textContent = 'Run cross-repo task'; btn.disabled = false;
    });
  }
  loadCrossRepo();
}

async function loadCrossRepo() {
  const data = await getJSON('/api/cross-repo');
  const fc = $('xrepo-feed'); clear(fc);
  if (!data.feed.length) fc.appendChild(el('div', 'feed-empty', 'No activity yet. Run the cross-repo task above.'));
  data.feed.forEach(e => {
    const dev = (e.developer || 'unknown').replace(/[^a-z-]/gi, '');
    const row = el('div', `feed-row ${e.action === 'created' ? 'is-write' : 'is-query'}`);
    const left = el('div', 'feed-left');
    left.appendChild(el('span', `feed-dev dev-${dev}`, e.developer === 'seed' ? 'baseline' : (e.developer || '?')));
    left.appendChild(el('span', `feed-action act-${e.action}`, e.action === 'created' ? 'WRITE' : 'QUERY'));
    left.appendChild(el('span', `repo-badge repo-${e.repo}`, e.repo));
    row.appendChild(left);
    row.appendChild(el('div', 'feed-detail', e.detail || ''));
    fc.appendChild(row);
  });

  const jc = $('xrepo-join'); clear(jc);
  data.join.forEach(r => {
    const row = el('div', 'xrepo-row' + (r.new ? ' is-new' : ''));
    const acct = el('div', 'xrepo-acct');
    acct.appendChild(el('span', 'repo-badge repo-lza', 'lza'));
    acct.appendChild(el('span', 'xrepo-acct-name', shortName(r.lza_account)));
    if (r.new) acct.appendChild(el('span', 'xrepo-new', 'new'));
    acct.appendChild(el('span', 'xrepo-ref', 'account_ref=' + r.account_ref));
    row.appendChild(acct);
    const res = el('div', 'xrepo-res');
    res.appendChild(el('span', 'repo-badge repo-pulumi', 'pulumi'));
    if (!r.resources.length) res.appendChild(el('span', 'xrepo-empty', '(none yet)'));
    r.resources.slice(0, 6).forEach(x => {
      const chip = el('span', 'xrepo-chip' + (x.new ? ' is-new' : ''));
      chip.appendChild(el('span', null, shortName(x.name)));
      chip.appendChild(el('span', 'xrepo-chip-type', x.type));
      res.appendChild(chip);
    });
    if (r.resources.length > 6) res.appendChild(el('span', 'xrepo-more', '+' + (r.resources.length - 6) + ' more'));
    row.appendChild(res);
    jc.appendChild(row);
  });
  $('xrepo-stat').textContent = data.created ? 'cross-repo task complete' : 'ready';
}
